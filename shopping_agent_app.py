...
for card in cards[:12]:
    try:
        ...
        results.append(ProductResult(title=title.strip(), price=price, url=href, rating=rating, availability=avail))
    except Exception as e:
...
for sel in [
    ...
    try:
        ...
        if await el.count():
            ...
            log.info(f"  -  '{await el.inner_text()}' button located -- STOPPING here (demo mode)")
            break
    except Exception:
        pass
