...
if text in query_lower or any(w in text for w in query_lower.split() if len(w) matched_href = await link.get_attribute("href")
...
rating = (await card.locator("p.star-rating").get_attribute("class") or "").rep
avail = (await card.locator("p.availability").inner_text()).strip()
results.append(ProductResult(title=title.strip(), price=price, url=href, rating=r
except Exception as e:
...
pass
# Variant selects
for vsel in ["select[name*='option']","select[name*='variant']","select[id*='size']","sel
try:
...
pass
# Add to cart -- comprehensive selector list
atc_selectors = [
...
# Confirm
try:
for csel in [".alert-success","p.alert","#basket-mini .badge",".flash.success","[class*='
...
# ------------------------------------------------------------------------------
async def _click_first(page: Page, selectors: list[str], label: str, log: logging.Logger) ->
for sel in selectors:
try:
el = page.locator(sel).first
if await el.count():
...
"""
Detect Stripe.js iframe(s) and fill card number, expiry, CVC inside them.
Stripe splits fields across up to three separate iframes in newer versions.
Returns True if at least the card number was filled.
"""
# Collect all Stripe-related frame locators
stripe_patterns = [
...
"iframe[title*='Card number']",
"iframe[aria-label*='card']",
]
stripe_frames = []
for pat in stripe_patterns:
locs = await page.locator(pat).all()
for loc in locs:
stripe_frames.append(page.frame_locator(
f"{pat}:nth-match({await page.locator(pat).all_inner_texts()})"
))
# Simpler approach: iterate page.frames directly
card_filled = False
...
try:
# Card number
for sel in ["input[name='cardnumber']", "input[placeholder*='1234']", "input[auto
...
log.info(f" card_filled = True
break
- Stripe card number: {cfg.card_number}")
except Exception as e:
log.debug(f" Stripe card number: {e}")
try:
for sel in ["input[name='exp-date']", "input[placeholder*='MM']", "input[autocomp
el = frame.locator(sel).first
if await el.count():
await el.click()
...
log.debug(f" Stripe expiry: {e}")
try:
for sel in ["input[name='cvc']", "input[placeholder*='CVC']", "input[placeholder*
el = frame.locator(sel).first
if await el.count():
await el.click()
await asyncio.sleep(0.15)
...
3. Advance to payment page
4. Fill all card fields (Stripe iframe or plain inputs)
5. STOP before final 'Place Order' click
"""
log.info("[STEP 4/4] Starting checkout flow...")
# -- 4a. Click Proceed to Checkout -----------------------------------------
checkout_btns = [
...
if not proceeded:
try:
await page.goto(f"{cfg.base_url}/checkout/shipping-address/", wait_until="domcont
...
pass
# -- 4c. Fill shipping / contact fields ------------------------------------
...
"input[name='billing_state']", "select[name='state']",
...
el = page.locator(sel).first
if await el.count():
place_order_text = f"FOUND but NOT clicked: '{await el.inner_text()}'"
log.info(f" - '{await el.inner_text()}' button located -- STOPPING here (de
break
except Exception:
pass
...
log.info(f"Intent - query={intent.query!r} max_price={intent.max_price} "
...
result["intent"] result["elapsed"] = elapsed
= {"query": intent.query, "max_price": intent.max_price}
log.info("-" * 56)
...
section[data-testid='stSidebar'] { background: #0d1220 !important; border-right: 1px section[data-testid='stSidebar'] * { color: #94a3b8 !important; }
solid
.agent-card {
...
div[data-testid='stTextArea'] textarea { background:#0d1220; color:#e2e8f0; border:1px soli
div[data-testid='stTextInput'] input { background:#0d1220; color:#e2e8f0; border:1px soli
.stButton>button {
...
brd_customer = st.text_input("BRD Customer ID", value=os.environ.get("BRD_CUSTOMER",""),
brd_zone = st.text_input("BRD Zone", value=os.environ.get("BRD_ZONE","scraping
brd_password = st.text_input("BRD Password", value=os.environ.get("BRD_PASSWORD",""),
...
done = any(kw.lower() in logs_text for kw in step_keywords[:i+1]) and i < len(STEPS
...
log_text = "\n".join(st.session_state.log_lines) if st.session_state.log_lines else "Wait
...
st.markdown(f"**Screenshot:** `{r.get('screenshot','--')}`")
if os.path.exists("checkout_ready.png"):
st.image("checkout_ready.png", caption="Final checkout state (before Place Or
st.markdown("</div>", unsafe_allow_html=True)
...
'AI Shopping Agent - Bright Data Hackathon - Playwright + Claude + Bright Data Scraping B
'</p>',
unsafe_allow_html=True,
)
